...
RUN zypper install -y tar \
 && cd /tmp \
 && tar -xvzf /tmp/zulu8.19.0.1-jdk8.0.163-linux_x64.tar.gz \
 && mkdir -p /usr/lib/jvm \
 && mv /tmp/zulu8.19.0.1-jdk8.0.163-linux_x64 /usr/lib/jvm/zulu-8-amd64 \
 && rm -f /tmp/zulu8.19.0.1-jdk8.0.163-linux_x64.tar.gz \
 && zypper remove -y tar \
 && zypper clean 
